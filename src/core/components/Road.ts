...
export class Road extends Component {
    ...
    update(dt: number): void {
        ...
        for (let mix of this.mixerList) {
            ...
            if (mix.time + dt > 1.4583333333333333) {
                ...
                // 暂停动画
                const mixerAny = mix as any;
                // _actions / _clip are internal fields of THREE.AnimationMixer
                if (this.shouldOpenDoor && mix.time + dt * 1.6 < mix.timeScale * mixerAny._actions[0]._clip.duration) {
                    mix.update(dt * 1.6)
                }
            }
            else {
    ...
    private cloneObject3D(object: Object3D) {
